...
{
	for each (Vehicle vehicle in vehicleType)
	{
...
int main()
{
	...
	std::thread threadOne(findVehicle, cars, searchTerm), threadTwo(findVehicle, trucks, searchTerm), threadThree(findVehicle, motorcycles, searchTerm);
	threadOne.join();
	threadTwo.join();
	threadThree.join();
